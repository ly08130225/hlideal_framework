...
label1:
	...
	private static final char SEPARATOR_CHAR_ASTERISK = 42;
	...
	public static boolean isMobile(String mobile)
	{
		...
		if (!isNumeric(mobile))
			return false;
		return mobile.startsWith("13") || mobile.startsWith("14") || mobile.startsWith("15") || mobile.startsWith("18") || mobile.startsWith("17");
	}
		...
		if (mode <= 0)
			...
			} else
			{
				if (stripChars.length() == 0)
					return str;
				for (; start < end && stripChars.indexOf(str.charAt(start)) != -1; start++);
			}
		if (mode >= 0)
			if (stripChars == null)
			{
				for (; start < end && Character.isWhitespace(str.charAt(end - 1)); end--);
			} else
			{
				if (stripChars.length() == 0)
					return str;
				for (; start < end && stripChars.indexOf(str.charAt(end - 1)) != -1; end--);
			}
...
label1:
	...
	private static Pair getMaskLength(int len)
	{
	...
	public static String abbreviate(String str, int offset, int maxWidth)
	{
		...
		if (offset <= 4)
			return (new StringBuilder()).append(str.substring(0, maxWidth - 3)).append("...").toString();
		if (maxWidth < 7)
			maxWidth = 7;
		if (offset + (maxWidth - 3) < str.length())
			return (new StringBuilder()).append("...").append(abbreviate(str.substring(offset), maxWidth - 3)).toString();
		else
			return (new StringBuilder()).append("...").append(str.substring(str.length() - (maxWidth - 3))).toString();
	}
	...
	public static int getNum(String origin, String ch)
	{
		...
		int index = 0;
		int count = 0;
		for (; origin.indexOf(ch, index) != -1; index = origin.indexOf(ch, index) + chLength)
			count++;
