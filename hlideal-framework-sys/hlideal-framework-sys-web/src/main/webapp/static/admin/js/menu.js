            ...
            if (item.children && item.children.length > 0) {
                ...
                $.each(item.children, function (j, app) {
                    ...
                    if (app.children.length == 0 && app.path !='#') {
                        html.push('<a onclick="openMenu(\'zd' + app.id + '\')" href="' + app.path + '" target="index">');
                    } else {
                        html.push('<a href="/admin/backstage/editMenu.htm?appId='+app.id+'" onclick="edit_Menu(\'zd'+app.id+'\')" target="index">');
                    }
                    html.push(app.label);
                    html.push('</a>');
                    //三级菜单
                    if (app.children && app.children.length > 0) {
                        ...
                        $.each(app.children, function (k, _menu) {
                            ...
                            if (_menu.children && _menu.children.length > 0) {
                                ...
                                $.each(_menu.children, function (l, _menu) {
                                    html.push('<li>');
                                    if (typeof (_menu.children)!='undefined' && _menu.children.length == 0 && _menu.path !='#') {
                                        html.push('<a onclick="openMenu(\'zd' + _menu.id + '\')" href="' + _menu.path + '" target="index">');
                                    } else {
                                        html.push('<a href="javascript:void(0);">');
                                    }
...
function edit_Menu(e) {
    var key = e.replace("zd", "");
    var data = userMenu[key];
    createTab(data.id, data.label+'菜单编辑', '/admin/backstage/editMenu.htm');
