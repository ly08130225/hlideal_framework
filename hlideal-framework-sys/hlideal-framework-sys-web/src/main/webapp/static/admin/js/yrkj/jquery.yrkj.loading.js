define(function (require, exports, module) {
	...
	Loading.zindex = 8888;
	Loading.html = '<div class="yrkj_loading" style="display: none"><div class="loading-pic"><img src="/static/admin/images/spinner.gif"><p>loading…</p></div> </div>';
	Loading.divHtml = '<div class="yrkj_div_loading" style="display: none"><div class="loading-pic"> <img src="/static/admin/images/spinner.gif"><p>loading…</p></div> </div>';
	...
	Loading.create = function (obj, showImage, zindex) {
		...
		//判断是否存在obj
		if (obj) {
			if (!($("#" + obj + " .yrkj_div_loading").length && $("#" + obj + " .yrkj_div_loading").length > 0)) {
				$("#" + obj).prepend(Loading.divHtml);
			}
			loadingObj = $("#" + obj + " .yrkj_div_loading").first();
			...
			//如果存在则需要修改zindex
			if ($(".yrkj_loading").length && $(".yrkj_loading").length > 0) {
				var z_index = $(".yrkj_loading").last().css("z-index");
				if (zindex < (z_index++)) {
					zindex = z_index++;
				}
			}
			$("body").prepend(Loading.html);
			loadingObj = $(".yrkj_loading").first();
			if (showImage === false) {
