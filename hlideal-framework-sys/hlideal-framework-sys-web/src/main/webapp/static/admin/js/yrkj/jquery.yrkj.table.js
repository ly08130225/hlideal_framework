define(function (require, exports, module) {
	var $ = require('jquery');
	var Loading = require('yrkj/jquery.yrkj.loading');
	var callBack = require('yrkj/jquery.yrkj.page.callback');
	require('yrkj/jquery.yrkj.button');
	...
	// 插件的定义
	$.fn.YTable = function (options, param) {
		//如果为string，表示是方法调用
		...
		options = options || {};
		var opts = $.extend({}, $.fn.YTable.defaults, options);
	...
	$.fn.YTable.methods = {
		...
		addRow: function (jq) {
			...
			$.each(options.columns[0], function (c, column) {
				...
				switch (type) {
					case 'text':
						tdHtml += '<input type="text" class="yui-input"  style="width:' + width + 'px;height:' + height + 'px;line-height:' + height + 'px;"/>';
						break;
					case 'date':
						tdHtml += '<input type="text" class="yui-date" onclick="WdatePicker()" readonly  style="width:' + width + 'px;height:' + height + 'px;line-height:' + height + 'px;"/>';
						break;
					case 'textarea':
						width = options.data.type[1].areaWidth;
						height = options.data.type[1].areaHeight;
						tdHtml += '<textarea class="yui-area" style="width:' + width + 'px;height:' + height + 'px"></textarea>';
						break;
					default:
						tdHtml += '<input type="text" class="yui-input"  style="width:' + width + 'px;height:' + height + 'px;line-height:' + height + 'px;"/>';
				}
		...
		editRow: function (jq) {
			...
			for (var i = 0; i < len; i++) {
				...
				$.each(options.columns[0], function (c, column) {
					...
					switch (type) {
						case 'text':
							tdObj.append('<input type="text" class="yui-input" value="' + tdText
								+ '" style="width:' + width + 'px;height:' + height + 'px;line-height:' + height + 'px;"/>');
							break;
						case 'date':
							tdObj.append('<input type="text" class="yui-date" onclick="WdatePicker()" readonly  value="' + tdText + '" style="width:' + width + 'px;height:' + height + 'px;line-height:' + height + 'px;"/>');
							break;
						case 'textarea':
							width = options.data.type[1].areaWidth;
							height = options.data.type[1].areaHeight;
							tdObj.append('<textarea class="yui-area" style="width:' + width + 'px;height:' + height + ' px" >' + tdText + '</textarea>');
							break;
					}
				});
			...
			$table.find('.datagrid-body').find("tr").unbind("click").bind("click", function () {
				...
				$(this).find(':input').each(function () {
					...
					if ($(this).attr('trSelect')) {
						...
						//控制全选按钮的状态
						if (options.rows == options.selectRows.length) {
						...
						//控制全选的状态(rows应该去掉表头row)
						if (options.rownumber-1 == options.selectRows.length) {
							header_input.attr('checked', 'checked');
							header_input.attr('trSelect', true);
						} else {
							header_input.removeAttr('checked');
	...
	function showPage(obj) {
		...
		html += '<input id="datagrid_currentPage_input" name="currentPage" type="text" class="yrkj-input" ' +
